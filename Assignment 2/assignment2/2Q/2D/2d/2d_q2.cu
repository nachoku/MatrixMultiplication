...
void element(int n, int row_num_a, int col_num_a, int col_num_b, float *a, float *b, float *c)
{
  ...
  if(row<col_num_b && col<row_num_a)
  {
    // printf("Thread: %d \n ", i);
    ...
    {
    //   printf("Block ID:(%d, %d)\nThread ID:(%d, %d)\nrow:%d - col:%d \n A: %d  \n B: %d  \n C: %d \n\n", blockIdx.x,blockIdx.y, threadIdx.x, threadIdx.y,row, col, col_num_a*row + j, col_num_b*j + col, row*col_num_b +col);
      temp+=a[col_num_a*row + j] * b[col_num_b*j + col];
    }
    c[row*col_num_b + col]=temp;
    // printf("%f ", c[quot*row_num:q_b + rem]);
  }
