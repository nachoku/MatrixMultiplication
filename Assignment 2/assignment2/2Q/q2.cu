...
__global__
void element(int n, int row_num_a, int col_num_a, int col_num_b, float *a, float *b, float *c)
{
  int row=blockIdx.x*blockDim.x+threadIdx.x;
  int col=blockIdx.y*blockDim.y+threadIdx.y;

  if(row<row_num_a && col<col_num_b)
  {
    // printf("Thread: %d \n ", i);
    ...
    {
  //     printf("row:%d - col:%d \n A: %d: %f \n B: %d: %f \n\n", row, col, col_num_a*row + j, a[col_num_a*row + j], col_num_b*j + col, b[col_num_b*j + col]);
      temp+=a[col_num_a*row + j] * b[col_num_b*j + col];
    }
    c[row*col_num_b + col]=temp;
...
int main(void)
{
  ...
  matrix_size.uiHA =  mul*320;
  matrix_size.uiWB =  mul*320;
  matrix_size.uiHB =  mul*160;
  matrix_size.uiWC =  mul*320;
  matrix_size.uiHC =  mul*320;
  ...
  // printf("maxthreadsperbloc:%d \n", deviceProp.maxThreadsPerBlock);
  // printf("maxThreadsDim[3]:%d \n", deviceProp.maxThreadsDim[3]);
  // printf("maxGridSize[3]:%d \n", deviceProp.maxGridSize[3]);
  int block_size = (deviceProp.major < 2) ? 16 : 32;

  dim3 threads(8, 8);
  dim3 grid(matrix_size.uiWC / threads.x, matrix_size.uiHC / threads.y);
  //dim3 grid(4,4);
  element<<<grid, threads>>>(N, matrix_size.uiHA, matrix_size.uiWA, matrix_size.uiWB, d_a, d_b, d_c);
