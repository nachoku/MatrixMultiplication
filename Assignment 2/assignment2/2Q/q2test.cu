...
void element(int n, int row_num_b, float *a, float *b, float *c)
{
  ...
  printf("Hello from X BlockId: %d, dim %d, thread %d: row: %d\nHello from Y BlockId: %d, dim %d, thread %d: col: %d\n\n", blockIdx.x, blockDim.x, threadIdx.x, row, blockIdx.y, blockDim.y, threadIdx.y, col);
...
int main(void)
{
  ...
  //element<<<1 threads, >>>(N, matrix_size.uiHB, d_a, d_b, d_c);
